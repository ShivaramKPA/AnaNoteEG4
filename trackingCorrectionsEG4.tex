...
This work is mostly based the work documented in the \href{https://www.jlab.org/Hall-B/secure/eg1-dvcs/technotes/targetinfo/target_info.pdf}{EG1-DVCS technical note \#{} 4}
...
(We tried to find $e^{+}e^{-}$ pairs in our data set to use in the
minimization just like in the EG1DVCS, but due to various reasons,
we were unable to find them, and, therefore, had to settle with the
ep-elastic events only).


\subsection{Method}

First the ep-elastic events are skimmed (from all of the \textbf{$NH_{3}$}
target data-set) using particle ID cuts for the electrons in the 6th
sector and protons detected in th 3rd sector and then using 4-momentum
conservation requirements (within measurement uncertainties), to help
further clean up the accidental coincidenes. These skimmed events
are saved in root files and later reused for the minimization process
described here.

In this method a correction routine involving a set of correction
equations with several unknown parameters are established first. Then
with the help of TMinuit (ROOT version of TMinuit), several sets of
...
the Minuit will try to find the optimum set of these parameter values
for which the \textbf{$\chi^{2}$}will be minimum. Such an optimal
set of values are chosen as the correct values of these parameters
and will be used in the eventual correction routine.


\subsection{The correction routine}

The routine uses 17 constants (free parameters determined by the process
...
\end{itemize}
\item Then, $\theta_{f}$ is corrected as follows:
...
\end{itemize}
\end{itemize}
\item Next, get $\phi_{0}$ without raster corrections yet 
...
values $E_{0}$ of beam energy\footnote{For the beam energies, average values 1.9911 and 1.3369 were used which were derived from the Hall A and Hall C Tiefenbach energies\cite{anaNoteXZheng} or the MDSY1c or MDSY3c energies or from Hall B Tiefenback energies whenver there was no measurement from the other halls for a given run.}.
...
\protect\caption{Comparing various quantities before and after the corrections.}
\end{figure}
...
\item $Z_{cor}=Z+\frac{X_{p}}{\tan(\theta)}+factor1\times factor2$ 
...
\item $factor2=\frac{1-0.25\times \tan\theta\times \tan\theta}{\sin\theta\times \cos\theta}$ 
