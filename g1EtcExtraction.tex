...
\end{equation}
%The only thing remaining is the evaluation of systematic errors due to the other possible sources of disagreement between data and simulation.

The values of \gones and its errors thus extracted from 1.3 GeV data for one \qsqs bin is shown in Fig. (\ref{g1q7}). Similar results for all the bins from two beam energy data sets in different kinematic bins can be seen in Fig. \ref{extG1}.% (next chapter). 
...
\subfigure[Difference $\Delta n(data) - \Delta n(sim)$ (for 1.3 GeV case).]{
...
%In this section, the methods to extract \gone and evaluate its errors are described.  %$g_{1}$
